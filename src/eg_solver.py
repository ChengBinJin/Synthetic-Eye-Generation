...
class Solver(object):
    def __init__(self, flags, log_dir=None):
        ...
        self._init_iden_model(flags)
        self._init_gen_model()

    def _init_iden_model(self, flags):
        self.iden_model = ResNet18(input_img_shape=(200, 200, 1),
                                  num_classes=self.data.num_identities,
                                  is_train=False)
        self.iden_model_dir = flags.load_iden_model

        flag, iter_time = self.load_model(os.path.join('../model/identification', self.iden_model_dir))
        if flag is True:
    ...
    # def train(self, evaluater=None):
    #     imgs, clses, segs = self.data.train_random_batch(batch_size=self.batch_size)
    #
